...
async function createStripeInfrastructure() {
  ...
  try {
    ...
    const mensualLink = await stripeClient.paymentLinks.create({
      line_items: [{ price: mensualPrice.id, quantity: 1 }],
    });
    const anualLink = await stripeClient.paymentLinks.create({
      line_items: [{ price: anualPrice.id, quantity: 1 }],
    });
    ...
    const esencialLink = await stripeClient.paymentLinks.create({
      line_items: [{ price: esencialPrice.id, quantity: 1 }],
    });
    ...
    const integralLink = await stripeClient.paymentLinks.create({
      line_items: [{ price: integralPrice.id, quantity: 1 }],
    });
    ...
    const premiumLink = await stripeClient.paymentLinks.create({
